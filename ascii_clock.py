...
def get_time_components():
    ...
    # Extend hours string if necessary to account for user not adding leading zero
    if len(hours) == 1:
        hours = "0" + hours
    return hours, minutes


def choose_clock_type():
...
def build_ascii_art_dict(preferred_character):
...
def render_ascii_clock(display_time, ascii_art_dict):
    # Print blank line for spacing
    print()
    # Print the ASCII clock by iterating through each row
    row_count = len(next(iter(ascii_art_dict.values())))
...
def main():
    hours, minutes = get_time_components()
    clock_type = choose_clock_type()
    display_time = format_display_time(hours, minutes, clock_type)
    preferred_character = get_preferred_character()
    ascii_art_dict = build_ascii_art_dict(preferred_character)
    render_ascii_clock(display_time, ascii_art_dict)
